...
public class _02_ScopeVariables {
	// 1. 클래스 변수(인스턴스 변수) 선언
	// 클래스 중괄호 안에서 어디서든지 접근해서 사용하거나 값을 저장할 수 있다.
	static int num;
	
	// 인스턴스 변수(static 키워드 없이 사용할 수 있는 변수)
	int instanceNum;
...
//		System.out.println(ifNum);
		...
		// 4-1. 메소드의 실행(호출)
		// localVal1, localVal2, localResults는 이때 메모리에 저장되었다가 메소드가 종료되는 시점에서 메모리에서 삭제
		getSum();
		// 메소드의 종료
		
		/* 지역변수는 메소드가 종료되면서 메모리에서 삭제되기 때문에 메모리에 남아있는 않는 변수에 접근하려고 해서 에러가 발생
